...
def main():
    ...
    active_images = list(
        map(lambda x: f"./images/{active}/{x}", os.listdir(f"./images/{active}/")))
    ...
    for folder in os.listdir('./images'):
        if len(folder.split(".")) == 1 and folder.split(".")[0] != active:
            rand_images.extend(
                [f"./images/{folder}/{image}" for image in os.listdir(f'./images/{folder}')])
    ...
    createImage_list()
    Button(master=galleryGrid[0], text=str(0), image=image_list[0], bd=0, highlightthickness=0, command=lambda: validate(
        0, galleryGrid[0]), relief=FLAT).place(relx=0.5, rely=0.5, anchor="center")
    Button(master=galleryGrid[1], text=str(1), image=image_list[1], bd=0, highlightthickness=0, command=lambda: validate(
        1, galleryGrid[1]), relief=FLAT).place(relx=0.5, rely=0.5, anchor="center")
    Button(master=galleryGrid[2], text=str(2), image=image_list[2], bd=0, highlightthickness=0, command=lambda: validate(
        2, galleryGrid[2]), relief=FLAT).place(relx=0.5, rely=0.5, anchor="center")
    Button(master=galleryGrid[3], text=str(3), image=image_list[3], bd=0, highlightthickness=0, command=lambda: validate(
        3, galleryGrid[3]), relief=FLAT).place(relx=0.5, rely=0.5, anchor="center")
    Button(master=galleryGrid[4], text=str(4), image=image_list[4], bd=0, highlightthickness=0, command=lambda: validate(
        4, galleryGrid[4]), relief=FLAT).place(relx=0.5, rely=0.5, anchor="center")
    Button(master=galleryGrid[5], text=str(5), image=image_list[5], bd=0, highlightthickness=0, command=lambda: validate(
        5, galleryGrid[5]), relief=FLAT).place(relx=0.5, rely=0.5, anchor="center")
    Button(master=galleryGrid[6], text=str(6), image=image_list[6], bd=0, highlightthickness=0, command=lambda: validate(
        6, galleryGrid[6]), relief=FLAT).place(relx=0.5, rely=0.5, anchor="center")
    Button(master=galleryGrid[7], text=str(7), image=image_list[7], bd=0, highlightthickness=0, command=lambda: validate(
        7, galleryGrid[7]), relief=FLAT).place(relx=0.5, rely=0.5, anchor="center")
    Button(master=galleryGrid[8], text=str(8), image=image_list[8], bd=0, highlightthickness=0, command=lambda: validate(
        8, galleryGrid[8]), relief=FLAT).place(relx=0.5, rely=0.5, anchor="center")
...
def verify_captcha():
    if sorted(position) == sorted(selected):
        msg.showinfo(
            "Success", message="You completed verification successfully")
    else:
...
def createImage_list():
    global image_list
    image_list = [ImageTk.PhotoImage(Image.open(
        rand_images[i]).resize((130, 130))) for i in range(9)]
